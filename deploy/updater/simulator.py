...
async def simulate_alerts_fusion_websocket_v1(mc):
    ...
    try:
        ...
        for region_id, flags in data.items():
            alert_types = []
            if flags & (1 << 0):
                alert_types.append("AIR")
            if flags & (1 << 1):
                alert_types.append("ARTILLERY")
            if flags & (1 << 2):
                alert_types.append("URBAN_FIGHTS")
            if flags & (1 << 3):
                alert_types.append("CHEMICAL")
            if flags & (1 << 4):
                alert_types.append("NUCLEAR")
            if flags & (1 << 5):
                alert_types.append("Drones")
            if flags & (1 << 6):
                alert_types.append("Missile")
            if flags & (1 << 7):
                alert_types.append("KAB")
            if flags & (1 << 8):
                alert_types.append("Ballistic")
            if flags & (1 << 9):
                alert_types.append("Explosion")
            if flags & (1 << 10):
                alert_types.append("Recon Drones")

            logger.info(f"Регіон {region_id}: flags={flags} ({bin(flags)}), типи: {', '.join(alert_types)}")

        # Зберігаємо дані через store_websocket_data
        if notifications:
            await store_websocket_data(
                mc, data, websocket, "etryvoga_fusion_websocket_v1", b"etryvoga_fusion_websocket_v1"
            )
        if alerts:
