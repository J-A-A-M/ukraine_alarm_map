...
async def stats(request):
    if request.path_params["token"] == data_token:
        ...
        tcp_clients_data = json.loads(tcp_clients.decode("utf-8", "surrogateescape")) if tcp_clients else {}

        websocket_clients = await mc.get(b"websocket_clients")
        websocket_clients_data = json.loads(websocket_clients.decode("utf-8", "surrogateescape")) if websocket_clients else {}
        websocket_clients_dev = await mc.get(b"websocket_clients_dev")
        websocket_clients_dev_data = json.loads(websocket_clients_dev.decode("utf-8", "surrogateescape")) if websocket_clients_dev else {}
        ...
        logger.warning(f"Stats: '{response}'")
