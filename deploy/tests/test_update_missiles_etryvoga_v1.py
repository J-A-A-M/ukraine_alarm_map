...
from updater.updater import update_websocket_v1_missiles
...
async def test_1(mock_get_redis_data, mock_set_redis_data):
    ...
    mock_pubsub.get_message = AsyncMock(
        side_effect=[{"type": "message", "channel": "alerts:etryvoga:missiles:updated", "data": "1"}]
    )

    async def get_redis_side_effect(_logger, _client, key, default_response=None):
        if key == "alerts:etryvoga:missiles:data":
            return {"11": "2022-02-24T03:40:00Z", "13": "2025-01-15T10:00:00Z"}
        elif key == "websocket:v1:legacy:missiles":
            return [1645674000] * LEGACY_LED_COUNT
        elif key == "websocket:v2:legacy:missiles":
            return [[0, 1645674000]] * LEGACY_LED_COUNT
        return default_response

    mock_get_redis_data.side_effect = get_redis_side_effect

    await update_websocket_v1_missiles(mock_redis, run_once=True)

    expected_result = [1645674000] * LEGACY_LED_COUNT
    expected_result[1] = 1736935200

    calls = [call for call in mock_set_redis_data.call_args_list if call[0][2] == "websocket:v1:legacy:missiles"]
    assert len(calls) > 0
...
async def test_2(mock_get_redis_data, mock_set_redis_data):
    ...
    async def get_redis_side_effect(_logger, _client, key, default_response=None):
        if key == "alerts:etryvoga:missiles:data":
            ...
            return [1736935200] * LEGACY_LED_COUNT
        elif key == "websocket:v2:legacy:missiles":
            return [[0, 1645674000]] * LEGACY_LED_COUNT
        return default_response
